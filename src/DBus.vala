...
private interface Notify.DBus : Object {
...
public class Notify.Server : Object {
    ...
    private Notify.Confirmation? confirmation = null;

    private GLib.Settings settings;

    private Gee.HashMap<uint32, Notify.Bubble> bubbles;
    ...
    construct {
        ...
        bubbles = new Gee.HashMap<uint32, Notify.Bubble> ();
    }
    ...
    public new uint32 notify (
        ...
        if (hints.contains (X_CANONICAL_PRIVATE_SYNCHRONOUS)) {
            ...
            var notification = new Notify.Notification (app_name, app_icon, summary, body, actions, hints);
            ...
            if (!settings.get_boolean ("do-not-disturb") || notification.priority == GLib.NotificationPriority.URGENT) {
                ...
                if (app_settings.get_boolean ("bubbles")) {
                    if (bubbles.has_key (id) && bubbles[id] != null) {
                        ...
                        bubbles[id] = new Notify.Bubble (notification, actions, id);
                        bubbles[id].show_all ();
    ...
    private void send_confirmation (string icon_name, HashTable<string, Variant> hints) {
        ...
        if (confirmation == null) {
            confirmation = new Notify.Confirmation (
                icon_name,
                progress_value
            );
